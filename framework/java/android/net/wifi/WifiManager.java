...
public class WifiManager {
    ...
    /**
     * @hide
     * We can't change IMS-facing API; so do some mapping here
     */
    private static HashMap<Integer, StaStateCallbackProxy> mStaStateCallbackList = new HashMap<>();
    /**
     * @hide
     */
	...
	Looper looper = (handler == null) ? mContext.getMainLooper() : handler.getLooper();
        mStaStateCallbackList.put(callback.hashCode(), new StaStateCallbackProxy(looper, callback));
        try {
            mService.registerStaStateCallback(mStaStateCallbackList.get(callback.hashCode()));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public void unregisterStaStateCallback(@NonNull StaStateCallback callback) {
        ...
        try {
            mService.unregisterStaStateCallback(mStaStateCallbackList.get(callback.hashCode()));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
	mStaStateCallbackList.remove(callback.hashCode());
    }
